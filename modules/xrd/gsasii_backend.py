...
def run_gsas2(tt, y_obs, sigma, phases, wavelength,
    ...
    try:
        ...
        # ── Per-phase patterns ─────────────────────────────────────────
        # Primary method: reconstruct per-phase profiles from each
        # phase's own reflection list (from generate_reflections, which
        # uses the refined cell parameters + space group) and the refined
        # instrument profile parameters.  Each phase gets its own
        # distinct set of reflections — this is reliable across all
        # GSAS-II versions.
        #
        # Fallback: ask GSAS-II to compute each phase's contribution by
        # temporarily zeroing every OTHER phase's scale and recomputing
        # with 0 refinement cycles.  This can fail depending on the
        # GSAS-II version.
        #
        ...
        if len(gsas_phases) > 1:
            ...
            # ── Primary: profile reconstruction from per-phase reflections ─
            # Uses the per-phase reflection lists already generated by
            # generate_reflections() (which correctly applies each phase's
            # own space group, cell parameters, and atom sites).  This
            # avoids the key-matching issues with GSAS-II's internal
            # Reflection Lists dict where phase names may not match.
