...
- Two-pass ghost reflection filter: absolute threshold (1e-4) + relative threshold (1% of max F²)
- Systematic absence rules implemented for 20+ space groups (Pbcn, P63/mmc, Pm-3n, Fd-3m, etc.)

### Tick Mark Generation (important design decision)
- **Tick positions** (vertical lines in result plot) are ALWAYS generated by `generate_reflections()` using pre-computed F² — this matches the preview and is reliable even when GSAS-II refinement hasn't converged
- **Phase profiles** (envelope curves) use GSAS-II's refined Fc² from the RefList when available, falling back to `generate_reflections()` output
- GSAS-II's Fc² can be unreliable for unconverged phases (e.g. carbides), so it must NOT be used for tick filtering — only for profile reconstruction
...
- `c408110` — Fix GSAS-II tick filter: threshold on raw Fc², not mult*Fc² (fixes WC P-6m2)
- `01b721b` — Raise ghost reflection threshold from 0.1% to 1% of strongest F²
- `f14560e` — Filter ghost tick marks from GSAS-II RefList + add CLAUDE.md
...
### Current session fixes (this branch: `claude/agitated-carson`):
- **Tick marks now always use `generate_reflections()`** — decoupled from GSAS-II RefList Fc² which can be unreliable for unconverged carbide fits
- **Ghost reflection threshold raised to 1%** — 0.1% was too permissive for W2C Pbcn
- **Stick pattern output filters `rel_int < 0.01`** — removes visual clutter in preview
- **GSAS-II RefList filters on raw Fc²** — not mult×Fc², preventing multiplicity from masking real reflections (WC P-6m2 fix)
